            ...
            AfterSheet::class => function (AfterSheet $event) {
                ...
                if ($this->grup != 'ya') {
                    ...
                    //  $event->sheet->mergeCells('G1:I1');
                     // $event->sheet->mergeCells($this->cellsToMergeByColsRow(0,2,3));
                     // $event->sheet->mergeCells(sprintf('A2:%s2',$last_column));
                     $event->sheet->mergeCells(sprintf('A%d:D%d', $last_row, $last_row));

                     // assign cell values

                     $event->sheet->setCellValue('A1', 'Customer');
                     $event->sheet->setCellValue('B1', 'Nama');
                     $event->sheet->setCellValue('C1', 'Subdept');
                     $event->sheet->setCellValue('D1', 'Pangkat');
                     $event->sheet->setCellValue('E1', 'Total');
                     $event->sheet->setCellValue('F1', 'Pembayaran');
                     $event->sheet->setCellValue('F2', 'Ekop');
                     $event->sheet->setCellValue('G2', 'Tunai');
                     $event->sheet->setCellValue('H2', 'Kredit');
                    //  // $event->sheet->setCellValue('J1','Lokasi');
                    //  // $event->sheet->setCellValue('K1','Lokasi');
                    //  // $event->sheet->setCellValue('L1','Lokasi');
                     $event->sheet->setCellValue(sprintf('A%d', $last_row), 'Total');
                     $event->sheet->setCellValue(sprintf('E%d', $last_row), $this->sumtotal);
                     $event->sheet->setCellValue(sprintf('F%d', $last_row), $this->sumekop);
                     $event->sheet->setCellValue(sprintf('G%d', $last_row), $this->sumtunai);
                     $event->sheet->setCellValue(sprintf('H%d', $last_row), $this->sumkredit);
