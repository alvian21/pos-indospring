...
@section('title', 'Transaksi | RUSAK HILANG')
            ...
            <div class="col-12">
                <div class="card card-dark">
                    <div class="card-header container-fluid d-flex justify-content-between">
                        <h4 class="text-dark"><i class="fas fa-list pr-2"></i> Transaksi | Rusak / Hilang</h4>
                                ...
                                <div class="col-md-9">
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control" id="keterangan" name="keterangan"
                                            rows="3">{{$trstockhilang["keterangan"]}}</textarea>
                                    </div>
                                </div>
                                <div class="col-3">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control ttl_harga_pajak" id="ttl_harga_pajak"
                                            value="{{$trstockhilang["total_harga_setelah_pajak"]}}" name="ttl_harga_pajak"
                                            readonly>
                                    </div>
                                </div>
                            </div>
    ...
    <div class="section-body">
        <div class="row">
            <div class="col-12">
                <div class="card card-dark">
                    <div class="card-header container-fluid d-flex justify-content-between">
                        <h4 class="text-dark"><i class="fas fa-list pr-2"></i> Detail Transaksi |  Rusak / Hilang</h4>
...
<script type="text/javascript">
    ...
    var table_detail = $("#table-detail").DataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: "{{ route('pos.stockhilang.datadetail') }}",
        columns: [
            {data: 'urut', name: 'urut'},
    ...
    $(document).on('keyup keydown','#supplier, #pajak, #lokasi, #keterangan', async function(){
       ...
       const result = await  $.ajax({
            url:"{{route('pos.transaksi_stockhilang.store')}}",
            method:"post",
            data:form
        });
    ...
    $('#barang').on('change', function () {
        ...
        if(kode_barang != 0){
            $.ajax({
            url:"{{route('pos.stockhilang.databarang')}}",
            method:"GET",
            data:{
        ...
        var qty = $('#qty').val();
            if(barang == '0'){
               $('.alert-danger').text('pilih barang terlebih dahulu')
                ...
                csrf_ajax();
                            $.ajax({
                                url:"{{route('pos.detail_transaksi_stockhilang.store')}}",
                                method: "POST",
      ...
      $(document).on('click','.btnDetailUpdate',function () {
        ...
        var qty = $('#qty').val();
            ...
            $('.alert-success').hide();
                ...
                csrf_ajax();
            $.ajax({
                url:"{{route('pos.detail_transaksi_stockhilang.update')}}",
                method: "POST",
                data: $('#formDetail').serialize(),
                success:function(data){
                    $('#formDetail').trigger("reset");
    ...
    $('.btnsimpan').on('click', function(){
        ...
        var cek = "";
        $.ajax({
            url:"{{route('pos.stockhilang.check')}}",
            async:false,
            method:"GET",
            success:function(data){
        ...
        if(cek){
            swal({
                ...
                .then((willInsert) => {
                if (willInsert) {
                    window.location.href = "{{route('pos.stockhilang.save')}}"
                }
            });
        }else{
    ...
    $(document).on('click','.btnDelete', function () {
       var urut = $(this).data('urut')
        ...
        swal({
            ...
            .then((willDelete) => {
            if (willDelete) {
               window.location.href="{{url('/admin/pos/stockhilang/delete_detail/')}}/"+urut;
            } else {
