                                ...
                                <div class="col-2">
                                    <div class="form-group">
                                        ...
                                        <input type="number" class="form-control" id="diskon_persen" min="0"
                                            value="{{$trmutasi["diskon_persen"]}}" name="diskon_persen" required>
                                    </div>
                                ...
                                <div class="col-2">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="diskon_rp" min="0"
                                            value="{{$trmutasi["diskon_rp"]}}" name="diskon_rp" required>
                                    </div>
                                ...
                                <div class="col-3">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="ttl_harga"
                                            value="{{$trmutasi["total_harga"]}}" name="ttl_harga" readonly>
                                    </div>
                                </div>
                                <div class="col-3">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control ttl_harga_pajak" id="ttl_harga_pajak"
                                            value="{{$trmutasi["total_harga_setelah_pajak"]}}" name="ttl_harga_pajak"
                                            readonly>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control" id="keterangan" name="keterangan"
                                            rows="3">{{$trmutasi["keterangan"]}}</textarea>
                                    </div>
                                </div>
                            </div>
                        </form>
    ...
    <div class="section-body">
        <div class="row">
            <div class="col-12">
                <div class="card card-dark">
                    <div class="card-header container-fluid d-flex justify-content-between">
                        <h4 class="text-dark"><i class="fas fa-list pr-2"></i> Detail Transaksi | Transfer Antar Toko</h4>
...
<div class="modal fade " id="totalModal" tabindex="-1" role="dialog" aria-labelledby="totalModalLabel"
    ...
    <div class="modal-dialog " role="document">
        <div class="modal-content">
            ...
            <form id="formTotal" method="POST" action="{{route('pos.mutasi.save')}}">
                @csrf
                <div class="modal-body">
                    <div id="alert-total">
...
<script type="text/javascript">
    ...
    var table_detail = $("#table-detail").DataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: "{{ route('pos.mutasi.datadetail') }}",
        columns: [
            {data: 'urut', name: 'urut'},
    ...
    $(document).on('keyup keydown','#supplier, #diskon_persen, #diskon_rp, #pajak, #lokasi, #keterangan', async function(){
       ...
       const result = await  $.ajax({
            url:"{{route('pos.transaksi_mutasi.store')}}",
            method:"post",
            data:form
        });
    ...
    $('#barang').on('change', function () {
        ...
        if(kode_barang != 0){
            $.ajax({
            url:"{{route('pos.mutasi.databarang')}}",
            method:"GET",
            data:{
        ...
        var qty = $('#qty').val();
            if(barang == '0'){
               $('.alert-danger').text('pilih barang terlebih dahulu')
                ...
                csrf_ajax();
               $.ajax({
                   url:"{{route('pos.detail_transaksi_mutasi.store')}}",
                   method: "POST",
      ...
      $(document).on('click','.btnDetailUpdate',function () {
        ...
        var qty = $('#qty').val();
            ...
            $('.alert-success').hide();
                ...
                csrf_ajax();
            $.ajax({
                url:"{{route('pos.detail_transaksi_mutasi.update')}}",
                method: "POST",
                data: $('#formDetail').serialize(),
                success:function(data){
                    $('#formDetail').trigger("reset");
    ...
    $(document).on('click','.btnDelete', function () {
       var urut = $(this).data('urut')
        ...
        swal({
            ...
            .then((willDelete) => {
            if (willDelete) {
               window.location.href="{{url('/admin/pos/mutasi/delete_detail/')}}/"+urut;
            } else {
